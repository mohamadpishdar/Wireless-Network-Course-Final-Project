...
EdcaTxopN::GetMinCw (void) const
{
      ...
      uint32_t fragmentSize = m_stationManager->GetFragmentationThreshold () - m_currentHdr.GetSize () - WIFI_MAC_FCS_LENGTH;
...
{
  NS_LOG_FUNCTION (this);
std::cout<<"khaaaaaar= "<<m_stationManager->GetFragmentSize (m_currentHdr.GetAddr1 (), &m_currentHdr,
                                              m_currentPacket, m_fragmentNumber)<<std::endl;

co++;

l=l+ m_stationManager->GetFragmentSize (m_currentHdr.GetAddr1 (), &m_currentHdr,
                                       m_currentPacket, m_fragmentNumber);


std::cout<<"kol meghdar= "<<l<<"shomare "<<co<<"taghsim "<<l/co<<std::endl;
