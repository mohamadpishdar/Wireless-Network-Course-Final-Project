...
uint32_t shomarande=0;
uint32_t uid=0;
bool uidsetcheck=false;
uint8_t ac ;
...
WifiRemoteStationManager::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::WifiRemoteStationManager")
    ...
    .AddAttribute ("TxopVi", "Txop time (in milliSeconds) for packets that have been tagged with AC_VI"
          "This value will not have any effect on some rate control algorithms.",
          TimeValue (Seconds (0.003008)),
          MakeTimeAccessor (&WifiRemoteStationManager::m_txopVI),
          MakeTimeChecker ())

          .AddAttribute ("TxopVO", "Txop time (in milliSeconds) for packets that have been tagged with AC_VO"
                "This value will not have any effect on some rate control algorithms.",
                TimeValue (Seconds (0.001507)),
                MakeTimeAccessor (&WifiRemoteStationManager::m_txopVO),
                MakeTimeChecker ())
          .AddAttribute ("TxopBK", "Txop time (in milliSeconds) for packets that have been tagged with AC_VI"
                "This value will not have any effect on some rate control algorithms.",
                TimeValue (Seconds (0.000508)),
                MakeTimeAccessor (&WifiRemoteStationManager::m_txopBK),
                MakeTimeChecker ())
          .AddAttribute ("TxopBE", "Txop time (in milliSeconds) for packets that have been tagged with AC_VI"
                "This value will not have any effect on some rate control algorithms.",
                TimeValue (Seconds (0.000008)),
                MakeTimeAccessor (&WifiRemoteStationManager::m_txopBE),
                MakeTimeChecker ())
                                           ...
                                           Ptr<const Packet> packet, uint32_t fullPacketSize)
{
    ...
    QosTag AccessCat;
   packet->PeekPacketTag(AccessCat);
                                           ...
                                           Ptr<const Packet> packet, uint32_t fullPacketSize)
{
 ...
 wf1=DoGetDataTxVector (Lookup (address, header), fullPacketSize).GetMode();
...
void
WifiRemoteStationManager::setUidForIdeal(uint32_t id)
{
uid=id;
uidsetcheck=true;
}
...
DoSetTxopThreshhold(m_txopVO);ac=3;
  ...
  shomarande++;
average_rate+=(GetCurrentRate (uid)/100000);
...
uint32_t
WifiRemoteStationManager::GetCurrentRate (uint32_t id){
    if (id==0)
return 1000000;
else if (id==1)
return 1000000;
...
std::cout<<"nfrag "<<nFragments<<std::endl;
...
void
WifiRemoteStationManager::DoSetTxopThreshhold(Time txop1){
uint32_t gam=GetCurrentRate (uid);
gam=gam/8;
SetFragmentationThreshold((txop1.GetSeconds()*gam));
